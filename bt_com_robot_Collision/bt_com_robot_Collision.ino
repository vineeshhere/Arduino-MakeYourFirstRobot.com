...
void TakeReverse() {
  /* putting the Robot in reverse mode for 1 second */
...
void loop() {
  ...
  /* Reads the serial port for bluetooth data, passed from smartphone */
   if(Serial.available() > 0) {
    data = Serial.read();
    Serial.println(data);
    
    distance = measureDistance();
    /* Check if there is any obstacles. If yes, shows face gesture of No  */
    if( data == '1' && distance > COLLISION_POINT)
      MoveRobotForward();
    else if(data == '1' && distance <= COLLISION_POINT)
      showFaceGestureNo();
